...
export default function CityStatsSection({ backgroundImage }: CityStatsSectionProps) {
  ...
  return (
    <section className="py-12 sm:py-20 relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-gray-900/90" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative" ref={ref}>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {stats.map((stat, index) => (
            ...
            >
              {/* Card */}
              <div className="relative bg-white/95 backdrop-blur-sm rounded-2xl p-4 sm:p-6 md:p-7 shadow-xl border border-white/20 hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 overflow-hidden">
                {/* Gradient Background */}
                <div
                ...
                <div className="relative mb-4 sm:mb-5">
                  <div
                    className={`w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-2xl bg-gradient-to-br ${stat.bgGradient} flex items-center justify-center transform group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300`}
                  >
                    <stat.icon className={`w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 ${stat.color}`} />
                  </div>
                </div>
                ...
                <div className="relative mb-1 sm:mb-2">
                  <div className="flex items-baseline gap-1">
                    <span className={`text-3xl sm:text-4xl md:text-5xl font-bold ${stat.color} tabular-nums`}>
                      <Counter value={stat.value} inView={inView} />
                    </span>
                    <span className={`text-2xl sm:text-3xl font-bold ${stat.color}`}>{stat.suffix}</span>
                  </div>
                </div>

                {/* Label */}
                <div className="relative">
                  <p className="text-gray-600 font-semibold text-base sm:text-lg">{stat.label}</p>
                </div>
